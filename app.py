...
def predictStaff(dataDict=None):
    if dataDict == None:
        app.logger.error('No Data Provided')
        return False
    # Classifiction Paramaters
    model = joblib.load('classsifier_staff.bin')
    ss = joblib.load('std_scaler_staff.bin')
    le = joblib.load('le_encoder_staff.bin')
    datalist = [
        (dataDict['Hospital_code'],
         dataDict['Hospital_type_code'],
         dataDict['City_Code_Hospital'],
...
@app.route('/patients_model_staff', methods=['GET', 'POST'])
def modelCheckStaff():
    if Checkpatients():
        dataDict = dict()
        if request.method == 'POST':
            ...
            dataDict['Admission_Deposit'] = request.form.get('deposit')
            app.logger.debug(dataDict)
            output = predictStaff(dataDict)
            return render_template('output_staff.html', message=output[0])
    else:
        return jsonify(
            {
                'error message': 'Authentication Unsuccessful, Login'
